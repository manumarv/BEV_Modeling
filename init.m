...
time_vector = (drivecycleData(:,1)).';
speed_vector = (drivecycleData(:, 3)).';
acceleration_vector = (drivecycleData(:, 4)).';

% Load vehicle parameters from Excel file
filename = 'Vehicles_Params_Summary.xlsx';
vehicleData = readtable(filename);

% List of vehicle makes to process
vehicleList = {'Renault Kangoo -E', 'Toyota_Proace', 'Peugeot E- Boxer', 'Blue Arc EV', 'Ford', 'Mercedes-Benz'};

% Initialize arrays to store results
motor_energy_all = zeros(length(vehicleList), 1);
battery_energy_all = zeros(length(vehicleList), 1);
...
G_cvt_min = 0.5; % minimum transmission gear ratio
...
dt = 0.1; % Define your simulation time step
...
% Display results
disp('Motor Energy (kWh):');
disp(motor_energy_all);

disp('Battery Energy (kWh):');
disp(battery_energy_all);
